...
// as we are storing everything in a single group...
// creating a static group...
string staticGid = "0_0";
...
int main(int argc, char *argv[])
{
	...
	cout << "Read Events from " << configInputFiles["eventsFile"] << "\n\n";
	...
	cout << "Reading docs from " << configInputFiles["docsFile"] << "\n\n";
	...
	cout << "Reading Map from " << configInputFiles["mapFile"] << "\n\n";
	...
	cout << "Getting Outdegrees from " << configInputFiles["outDegreeFile"] << "\n\n";
	outDegreeMap = getDegreeMap(configInputFiles["outDegreeFile"]);

	// inDegreeMap = getDegreeMap("../indegree_nodeid.txt");
	cout << "Getting Indegrees from " << configInputFiles["inDegreeFile"] << "\n\n";
	...
	cout << "Getting possible parents from  " << configInputFiles["top100CandidateParentsFile"] << " " << configInputFiles["top100CandidateParentsExpFile"] << "\n\n";
	...
	cout << "Writing log likelihood to " << configOutputFiles["logLikelihood"] << "\n\n";
...
vector< vector <li> > initializeForSampler(vector< vector <li> > allEvents, int flag)
{
	...
	for(ui i = 0; i < allEvents.size(); i++)
	{
		...
		// taking the nodeEvents counts for the Wuv denominator only from the first 15% of the events
		if (i <= 150000)
		{
...
int initializeUserUserInfluence()
{
	...
	for(ui i = 0; i < followersMap.size(); i++)
	{
		...
		for(ui j = 0; j < followers.size(); j++)
		{
			...
			groupSourceTransactionsSum[staticGid] += nodeEventsCountMap[uNode];
			totalVal += nodeEventsCountMap[uNode];

			if(nodeEventsCountMap[uNode] == 0)
			{
				// zeroSourceCount[gid] += 1;
				zeroSourceCount[staticGid] += 1;
			}

			// edgesInGroup[gid] += 1;
			edgesInGroup[staticGid] += 1;
			// numTimesAddedNuToGroup[gid] += 1;
			numTimesAddedNuToGroup[staticGid] += 1;
			numTimesAddedNu += 1;
	...
	// cout << "After intialization -- Nu Group Sum = " << groupSourceTransactionsSum[staticGid] << endl;
	cout << "Total Sum = " << totalVal << endl;

	long int calcSum = 0;

	// unordered_map<string, int>::iterator gstIt; 

	unordered_map<string, int>::iterator gstIt; 	//groupSourceTransactionsSum

	for(gstIt = groupSourceTransactionsSum.begin(); gstIt != groupSourceTransactionsSum.end(); gstIt++)
	{
		calcSum += gstIt->second;
	}

	cout << "Calculated Sum = " << calcSum << endl;
...
int sampleInfluenceAssignment(int ITE)
{
	...
	for(nodeNodeCountIterator = nodeNodeCount.begin(); nodeNodeCountIterator != nodeNodeCount.end(); nodeNodeCountIterator++)
	{
		...
		for(nodeCountMapIterator = nodeCountMap.begin(); nodeCountMapIterator != nodeCountMap.end(); nodeCountMapIterator++)
		{
			...
			// However, for a single (static) group case this wont happen...
			if(groupSourceTransactionsSum[staticGid] != 0)
			{
				// uinf = (groupTransactionsSum[gid] + baseAlpha) * 1.0 / (groupSourceTransactionsSum[gid] + baseBeta);
				uinf = (groupTransactionsSum[staticGid] + baseAlpha) * 1.0 / (groupSourceTransactionsSum[staticGid] + baseBeta);
			}
			// assign the updated value for the group...
			groupWuv[staticGid] = uinf;
	...
	cout << "Avg Wuv = " << avgVal / count << endl;
	cout << " Nuv of Group = " << groupTransactionsSum[staticGid] << " Nu Group Sum = " << groupSourceTransactionsSum[staticGid] << endl;
...
int updateNodeNodeCountMap()
{
	...
	// get the updated node node count...
	for(unsigned int i = 0; i < newSyntheticEvents.size(); i++)
	{
		eventNode = newSyntheticEvents[i][1];
		parentEvent = newSyntheticEvents[i][2];
	...
	for(nodeNodeCountIterator = nodeNodeCount.begin(); nodeNodeCountIterator != nodeNodeCount.end(); nodeNodeCountIterator++)
	{
		...
		for(nodeCountMapIterator = nodeCountMap.begin(); nodeCountMapIterator != nodeCountMap.end(); nodeCountMapIterator++)
		{
			...
			groupTransactionsSum[staticGid] += Nuv;

			// this is not required...
			// actualEdgesNuSum[gid] += Nu;
			actualEdgesNuSum[staticGid] += Nu;
			...
			triggeredEdges[staticGid] += 1;
		}
	}


	cout << "After Node-Node Count Update Nuv of Group = " << groupTransactionsSum[staticGid] << endl;
...
int writeGroupedUserUserInf()
{
	...
	for(groupTransactionsSumIterator = groupTransactionsSum.begin(); groupTransactionsSumIterator != groupTransactionsSum.end(); groupTransactionsSumIterator++)
	{
		...
		int nuDenomSum = groupSourceTransactionsSum[staticGid];
		...
		gUUInfFile << staticGid << " " << uinf << " " << groupTransactionsSum[staticGid] << " " << groupSourceTransactionsSum[staticGid] << "\n";
	}
