...
int createWordHistForDescendats();
...
int decreamentCountsFromDescendantEvents(li eventTopic, li eventIndex);
...
int increamentCountsForDescendantEvents(li eventTopic, li eventIndex);
...
unordered_map<ui, vector<ui> > wordHistForAllSpontaneousEvents;
unordered_map<ui, vector <ui> > combinedWordsForSponEventsDescendants;
...
vector <ui> spontaneousEvents, treeIdOfEvent;
unordered_map<ui, vector<ui> > spontaneousTreeMap;
...
int main(int argc, char *argv[])
{
	...
	cout << "Node node count initial update\n";
	
	initializeUserUserInfluence();
	cout << "initialized user user influence...with followers map...\n";
	...
	// create hist of words for words in the decendants and the spontaneous event...
	createWordHistForDescendats();
	...
	for(ui ITE = 0; ITE < ITERATIONS; ITE++)
	{
		...
		t2 = high_resolution_clock::now();
		duration = duration_cast<microseconds>( t2 - t1 ).count();
		cout << "\nUpdating Child Events Map and Node-Node Count after -- "<< ITE << " Iteration --  Time  taken -- " << duration << endl;
		...
		duration = duration_cast<microseconds>( t2 - t1 ).count();
		cout << "\nSampled User-User Influence for iteration -- "<< ITE << "--  Time taken -- " << duration << endl;
		...
		duration = duration_cast<microseconds>( t2 - t1 ).count();
		cout << "\nUpdating Alpha Value and User Base Rates -- "<< ITE << "--  Time taken -- " << duration << endl;
        cout << "\nDefault Mu Val = " << defaultMuVal << endl;
		...
		duration = duration_cast<microseconds>( t2 - t1 ).count();
		cout << "\nGot the hist of words for all the spontaneous events -- "<< ITE << "--  Time taken -- " << duration << "\n\n";
...
vector< vector <li> > initializeForSampler(vector< vector <li> > allEvents, int flag)
{
	...
	for(ui i = 0; i < allEvents.size(); i++)
	{
		...
		if(allPossibleParentEvents[i].size() > 0)
		{
			...
			int parentEventTopic = localNewSyntheticEvents[possParentEventId][3];

			tempEvent.push_back(parentEventTopic);
		}
		else
		{
			...
			tempEvent.push_back(i % numTopics);

			// keep track of spontaneous events...
			spontaneousEvents.push_back(i);
		}
	...
	cout << "\nUpdating the count matrices .... for the initialization...\n\n";
	...
	vector<int> countTopics(numTopics, 0);

	for(ui i = 0; i < localNewSyntheticEvents.size(); i++)
	{
		countTopics[localNewSyntheticEvents[i][3]] += 1;
	}
	
	cout << "After initializations...\n";
...
int updateNodeNodeCountMap()
{
	...
	nodeNodeCount.clear();
	// update spontaneous events at each iteration...
	spontaneousEvents.clear();
	...
	for(unsigned int i = 0; i < newSyntheticEvents.size(); i++)
	{
		...
		if(parentEvent >= 0)
		{
			...
			treeIdOfEvent.push_back(treeIdOfEvent[parentEvent]);
			spontaneousTreeMap[treeIdOfEvent[parentEvent]].push_back(i);
		}
		else
		{
			spontaneousEvents.push_back(i);
			treeIdOfEvent.push_back(i);
		}
	}
	...
	cout << "Spontaneous events having tree --- " << spontaneousTreeMap.size() << "\n\n";
	cout << "Spontaneous events -- " << spontaneousEvents.size() << "\n\n";
...
int sampleTopicAssignment(int ITE)
{
	...
	for(ui i = 0; i < spontaneousEvents.size(); i++)
	{
		li eventIndex = newSyntheticEvents[spontaneousEvents[i]][0];
		// li eventIndex = i;
		li eventNode = newSyntheticEvents[spontaneousEvents[i]][1];
		li eventParent = newSyntheticEvents[spontaneousEvents[i]][2];			// should be -2 for the first iteration...
		li eventTopic = newSyntheticEvents[spontaneousEvents[i]][3];				// should be -2 for the first iteration...

		vector<ui> combinedDoc = combinedWordsForSponEventsDescendants[spontaneousEvents[i]];

		li assignedTopic;


		decreamentCountFromMatrices(eventIndex, eventNode, eventParent, eventTopic, combinedDoc, 1);
		assignedTopic =  getSampledTopicAssignment(eventIndex, eventNode, eventParent, combinedDoc, ITE);
		if (assignedTopic == -1)
		{
			assignedTopic = eventTopic;
		}
		increamentCountToMatrices(eventIndex, eventNode, eventParent, assignedTopic, combinedDoc, 1);
		...
		// cout << "Spon event  -- "  << spontaneousEvents[i] << " i = " << i << "\n";
	}

	// change the topic for the descendants of each tree to the topic of the spontaneous event of that tree...
	for(unsigned int i = 0; i < newSyntheticEvents.size(); i++)	
	{
		...
		if(eventParent != -1)
		{
			assignedTopic = newSyntheticEvents[eventParent][3];
			newSyntheticEvents[i][3] = assignedTopic;
	...
	cout << logLikelihood << " LogLikelihood \n\n";
	printTopicTopicCount();
...
int printTopicTopicCount()
{
	...
	for(ui i = 0; i < numTopics; i++)
	{
		for(ui j = 0; j < numTopics; j++)
		{
			...
			if(i != j && NTTCountVec[i][j] != 0)
			{
				cout << "Some problem with topic-topic vector...\n";
				exit(0);
			}
		}
		cout << "\n";
	...
	cout << "Topic Topic matrix is still diagonal... :) \n";
...
li getSampledTopicAssignment(li eventIndex, li eventNode, li eventParent, vector<ui> combinedDoc, int ITE)
{
	li assignedTopic = -1;
	...
	vector<ui> wordHistVec = wordHistForAllSpontaneousEvents[eventIndex];
	// cout << "No. of Words = " << combinedDoc.size() << "\n\n";
	...
	if(wordHistVec.size() > 0)
	{
		for(li topic = 0; topic < numTopics; topic++)
		{
			...
			// if(combinedDoc.size() == 277385)
			// {
			// 	cout << "Topic id -- " << topic << "\n";
			// }

			thirdTerm = getThirdTermOfTopicAssignmentCondProb(topic, combinedDoc, wordHistVec);
			// thirdTerm = 0;
...
double getFirstTermOfTopicAssignmentCondProb(li eventNode, li eventParent, li topic)
{
	...
	if(eventParent != -1)
	{
		cout << "Something wrong with parent assignment... or recalculating the spontaneous event maps...\n\n";
		exit(0);
	}
...
/*
double getThirdTermOfTopicAssignmentCondProb(li topic, vector<ui> combinedDoc, vector<ui> wordHistVec)
{
	...
	long double finalThirdValueNume = 0;
	long double finalThirdValueDenom = 0;
	...
	while(i < wordHistVec.size())
	{
		...
		for(ui j = 0; j < wordCount; j++)
		{
			...
			currval += log((baseterm + j) / (baseTermDenom + k));
		...
		// if(combinedDoc.size() == 277385 && topic == 99)
		// {
		// 	cout << "ThirdTerm Numerator--  " << finalThirdValueNume << "\n";
		// }

		// finalThirdValueNume += log(valueForWord);
		finalThirdValueNume += currval;
		i++;
	}

	finalThirdValue = finalThirdValueNume;
...
double getThirdTermOfTopicAssignmentCondProb(li topic, vector<ui> combinedDoc, vector<ui> wordHistVec)
{
	...
	while(i < wordHistVec.size())
	{
		...
		// long double valueForWord = 1.0;
		long double valueForWord = 0;
		long double testval = 1;

		for(ui j = 0; j < wordCount; j++)
		{
			// valueForWord *= (baseterm + j);
			valueForWord += log(baseterm + j);
		...
		if(isinf(finalThirdValueDenom))
		{
			cout << "Going to infinity while calculating numerator... -- " << topic << "\n";
			exit(0);
		}
	...
	// if(topic == 99)
	// {
	// 	cout << "Numerator = " << finalThirdValueNume << "\n";
	// 	cout << "Baseterm Denom = " << baseTermDenom << "\n";
	// }

	for(ui i = 0; i < combinedDoc.size(); i++)
	{
		// finalThirdValueDenom *= (baseTermDenom + i);
		finalThirdValueDenom += log((baseTermDenom + i));
		if(isinf(finalThirdValueDenom))
		{
			cout << "Going to infinity while calculating denominator... -- " << topic << "\n";
			exit(0);
		}
	}
	...
	// {
	// 	cout << "Numerator = " << finalThirdValueNume << "\n";
	// 	cout << "Denominator = " << finalThirdValueDenom << "\n";
	// }
...
int createWordHistForDescendats()
{
	wordHistForAllSpontaneousEvents.clear();
	combinedWordsForSponEventsDescendants.clear();

	unordered_map<ui, vector <ui> >::iterator spontaneousTreeMapIterator;
	
	int countSpon = 0;

	for(spontaneousTreeMapIterator = spontaneousTreeMap.begin(); spontaneousTreeMapIterator != spontaneousTreeMap.end(); spontaneousTreeMapIterator++)
	{
		ui sponEvent = spontaneousTreeMapIterator->first;
		vector <ui> sponDescendants = spontaneousTreeMapIterator->second;

		vector <ui> sponDoc = allDocs[sponEvent];

		vector <ui> combinedDoc;

		for(ui i = 0; i < sponDoc.size(); i++)
		{
			combinedDoc.push_back(sponDoc[i]);
		}

		sponDoc.clear();

		for(ui eid = 0; eid < sponDescendants.size(); eid++)
		{
			sponDoc = allDocs[sponDescendants[eid]];
		...
		combinedWordsForSponEventsDescendants[sponEvent] = combinedDoc;

		unordered_map<ui, ui> wordHist;
		unordered_map<ui, ui>::iterator wordHistIt;
		wordHist = getHistOfWordsOverWordsFromDoc(combinedDoc);
		...
		wordHistForAllSpontaneousEvents[sponEvent] = tempWordCounts;

		countSpon += 1;
 	}

 	cout << "\nCreated hist for " << countSpon << " events...\n\n";
...
vector<double> populateCalculatedProbVec(vector <ui> possibleParentEvents, vector<double> possibleParentExp, li eventNode, li eventTopic, double eventTime, int ITE)
{
	...
	for(k = 0; k < possibleParentEvents.size(); k++)
	{
		...
		if(possParentEventTopic == eventTopic)
		{
...
// Decreament Counters
int decreamentCountFromMatrices(li eventIndex, li eventNode, li eventParent, li eventTopic, vector <ui> combinedDoc, bool topicSampling)
{
	...
	if(eventTopic >= 0)
	{
		...
		if(eventParent >= 0)
		{
			...
			if(eventParentTopic == eventTopic)
			{
				decreamentCountFromTopicTopic(eventParentTopic, eventTopic);
			}
			else
			{
				cout << eventIndex << " " << eventParentTopic << " " << eventTopic << "\n";
				cout << "Some thing wrong here... parentTopic != eventTopic while decreamenting...\n";
				exit(0);
			}
			// printUnorderedMap("ttopic");
		}
...
int decreamentCountFromTopicWord(li eventTopic, vector <ui> combinedDoc)
{
	if(NTWSumWordsVec[eventTopic] >= combinedDoc.size())
	{
		NTWSumWordsVec[eventTopic] -= combinedDoc.size();
	}
	else
	{
	...
	for(unsigned int i = 0; i < combinedDoc.size(); i++)
	{
		if(NTWCountVec[eventTopic][combinedDoc[i]] > 0)
		{
			NTWCountVec[eventTopic][combinedDoc[i]] -= 1;
		}
	}
...
{
	// vector <li> childEventsList;
	vector <ui> descendantsList;
	try
	{
		descendantsList =  spontaneousTreeMap.at(eventIndex);

		if(NTTCountVec[eventTopic][eventTopic] > 0)
		{
			NTTCountVec[eventTopic][eventTopic] -= descendantsList.size();
		}

		if(NTTSumTopicsVec[eventTopic] > 0)
		{
			NTTSumTopicsVec[eventTopic] -= descendantsList.size();
		}
	}
	catch(exception &e)
	{
...
// Increament Counters
int increamentCountToMatrices(li eventIndex, li eventNode, li eventParent, li eventTopic, vector<ui> combinedDoc, bool topicSampling)
{
	...
	if(eventParent >= 0)
	{
		...
		{
			cout << "Something wrong here... the parentTopic != eventTopic... increament would be wrong...\n";
			exit(0);
		}
		// printUnorderedMap("ttopic");
	}
	...
	if(topicSampling == 1)
	{
		...
		increamentCountInTopicWord(eventTopic, combinedDoc);
		...
		increamentCountsForDescendantEvents(eventTopic, eventIndex);
	}
...
int increamentCountInTopicWord(li eventTopic, vector<ui> combinedDoc)
{
	NTWSumWordsVec[eventTopic] += combinedDoc.size();
	// NTSumWords[eventTopic] += combinedDoc.size();
	...
	for(unsigned int i = 0; i < combinedDoc.size(); i++)
	{
		...
		if(NTWCountVec[eventTopic][combinedDoc[i]] > totalWords)
		{
			cout << "Total words = " << totalWords << " NTW = " << NTWCountVec[eventTopic][combinedDoc[i]] << " " << combinedDoc[i] << "\n";
			exit(0);
		}
	}
...
int increamentCountsForDescendantEvents(li eventTopic, li eventIndex)
{
	...
	try
	{
		...
		if(NTTCountVec[eventTopic][eventTopic] > newSyntheticEvents.size())
		{
			cout << "Somethings wrong.. the size of NTT is more than number of events...\n";
			cout << NTTCountVec[eventTopic][eventTopic] << " " << newSyntheticEvents.size() << "\n";
			exit(0);
		}

		if(NTTSumTopicsVec[eventTopic] > newSyntheticEvents.size())
		{
			cout << "Somethings wrong.. the size of NTT (sum) is more than number of events...\n";
			cout << NTTSumTopicsVec[eventTopic] << " " << newSyntheticEvents.size() << "\n";
			exit(0);
		}
	}
	catch(exception &e)
...
int getSampleFromMultinomial(vector<double> calculatedProbVec)
{
	...
	if(isnan(log(normalizedProbVector[assignedInd])))
	{
		cout << "Here is the problem.... -- " << normalizedProbVector[assignedInd] << "\n\n";

		for(ui i = 0; i < normalizedProbVector.size(); i++)
		{
