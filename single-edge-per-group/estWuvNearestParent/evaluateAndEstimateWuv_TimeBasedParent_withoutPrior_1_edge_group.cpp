...
int main()
{
	...
	predParentsFile.open("../top100CandParents_sample_events_1L_time_15.txt");
	...
	predValuesFile.open("origPredWuvValues_withoutPrior.txt");
	...
	for(edgeMarkWuvIt = edgeMarkWuv.begin(); edgeMarkWuvIt != edgeMarkWuv.end(); edgeMarkWuvIt++)
	{
		...
		if(NuValuesVec[uNode] > 0)
		{
			for(interMapIt = interMap.begin(); interMapIt != interMap.end(); interMapIt++)
			{
				...
				double predUVInf = ((predNuv[uNode][vNode]) * 1.0 ) / (NuValuesVec[uNode]);
